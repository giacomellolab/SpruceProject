...
```{r echo=FALSE}
spruce.merged.PCA.harmony <- readRDS("~/Documents/23-02-2021_spruce_new/spruce_merged_PCA_harmony_Umap_FindNeigh_findClusters.rds")
```
...
OBS! 'spruce.fill' did not work for visualising clusters from dimplots for some reason but worked well when applied to FeaturePlots and FeatureOverlays

Custom color palette - locked on 23-03-2021, 'spruce_color_new'??
...
spruce_color_new <- readRDS("~/Documents/shiny_spruce/spruce_color_new.rds")
```
...
```{r}
infotable <- read.csv("/Users/yuvarani.masarapu/Documents/23-02-2021_spruce_new/new_analysis_local/figs_28dims/infotable.csv")
infotable <- infotable[order(infotable$timepoint), ]
#I do ordering because my initial infotable was timepoint-sorted and I want the bud names to be sorted as well; this is done in order to get the right names when images are saved.
...
for ( i in 1:length(unique(spruce.merged.PCA.harmony@meta.data$section_id))){
  filename = paste0("/Users/yuvarani.masarapu/Documents/23-02-2021_spruce_new/new_imgs_23-03-2021/", bud.names[i], "_section" ,i ,"_clusters_onHE.png", sep = "")
...
```{r}
#custom plot, written by Åsa Björklund
...
for (i in 1:length(arrayid.names))
{
  ...
  filename = paste0("/Users/yuvarani.masarapu/Documents/23-02-2021_spruce_new/clusters_perSubArray/", arrayid.names[i],    "_allSections.png", sep = "")
...
3. Find marker genes from all clusters
...
4. Plot clusters for each section separately for better visualisation
...
name = paste0("/Users/yuvarani.masarapu/Documents/23-02-2021_spruce_new/indvClusters_perSection/",bud.names[i], "_section",i, "_indv_clusters.png")
ggsave(name, img1)
invisible(dev.off())
}
...
5. Plotting clusters on Grey/faded/white background
...
for (i in 1:length(unique(spruce.merged.PCA.harmony$section_id))){
  ...
  file.name <- paste("/Users/yuvarani.masarapu/Documents/23-02-2021_spruce_new/featureplots/", bud.names[i],"_section",i, "_clusters_greyBackground.png", sep = "")
